...
This file describes the current Bolt repo shape and the engineering principles to apply when changing it.

## Project Map

This repository is `bolt`: a local Apple-Silicon Zig + Metal parity workbench
with two active source packages and local-only reference assets.

```
bolt/
├── README.md                                  Concise project entry point
├── CODEX.md                                   Agent/developer project notes
├── .gitignore                                 Project-wide ignores
├── data/                                      Ignored local datasets/model assets
├── engine/
│   ├── build.zig                              Zig build for the Bolt engine
│   └── src/
│       ├── root.zig                           Public engine module
│       ├── network.zig                        Core network primitives
│       ├── transformer.zig                    Transformer implementation
│       ├── tokenizer.zig                      Tokenizer implementation
│       ├── safetensors.zig                    Safetensors parser
│       ├── weights.zig                        Weight-loading helpers
│       ├── q4.zig                             Quantization helpers
│       ├── bonsai_model.zig                   Bonsai model utilities
│       ├── cli/                               Local proof, fixture, bench CLIs
│       ├── fixtures/                          Manifest/payload contracts
│       ├── metal/                             Metal bridge, shader catalog, kernels
│       ├── models/                            Decoder and MNIST model paths
│       ├── runtime/                           Runtime assets, proof runs/checks, reports
│       ├── tensor/                            Tensor buffer/layout code
│       └── testing/                           Embedded deterministic sample constants
├── labrat/
│   ├── build.zig                              Zig build for Labrat tools/tests
│   ├── build.zig.zon                          Package manifest
│   ├── programs/                              Agent system prompts
│   └── src/
│       ├── agent_core.zig                     API-shaped offline/live agent core
│       ├── agent_cli_core.zig                 Shared agent CLI scenario runner
│       ├── api_client.zig                     Anthropic-compatible HTTP client
│       ├── toolbox.zig                        Sandboxed toolbox implementation
│       ├── tools.zig                          Shared CLI/file helpers
│       ├── researcher_core.zig                Researcher gate summary writer
│       ├── *_agent.zig                        Domain agent entry points
│       └── *_researcher.zig                   Domain researcher gate entry points
```

Current shape:

- `engine/` is the runtime and proof engine. It owns Zig + Metal kernels,
  model/fixture loaders, local CLIs, and embedded deterministic sample names.
  Its default health gate is `cd engine && zig build test`, backed by
  source-local Zig `test` blocks rather than a root `tests/` tree.
- `labrat/` is the sandboxed research/agent harness. It owns offline agent
  scenarios, live-mode blocking, researcher summaries, and toolbox tests.
- `data/` and local reference checkout roots are local-only input/reference
  material and must remain ignored. Do not commit real datasets, model weights,
  provider transcripts, or local reference checkouts.
- `research/` is optional local/reference material, not a default source package.
  If any research script is revived, derive it from current `engine/` and
  `labrat/` behavior rather than stale layouts.
- Root `tests/` is intentionally absent; do not rebuild a dedicated test folder
  unless the project policy changes.

### Real Bonsai/Q4 asset contract

Default engine checks are portable and must not require large model files.  Real
Bonsai/Q4 behavior is opt-in and becomes claimable only when a caller provides a
complete manifest at `artifacts/assets/bolt-parity/asset-manifest.json` or passes
an explicit manifest path after `--` to the relevant `zig build` step.

The manifest gate (`cd engine && zig build validate-assets`) requires concrete
entries for:

- `mnist-raw` — dataset files used by MNIST parity gates.
- `bonsai-1.7b` — full-precision model with `config.json`, `tokenizer.json`,
  and at least one parseable `.safetensors` file.
- `qwen3-1.7b-q4-gs64` — reference-compatible Q4 model with `config.json`,
  `tokenizer.json`, and parseable `.safetensors` tensors.

Each asset entry must include concrete `id`, `status`, `localPath`, `source`,
`provenance`, `licenseOrAccess`, `acceptanceUse`, per-file `path`, `role`,
`sha256`, and optional exact `size`; model entries also need
`modelTokenizerPairing` with `revision`, `sourceRepositoryOrUrl`, and `rule`.
Allowed manifest paths are limited to repo-local `data/` and `artifacts/`
entries or explicit `~/models/` entries.  Absolute paths, `.`/`..` segments,
and doubled separators are rejected; `~/models/` resolves through `$HOME` while
real datasets/model weights remain ignored under `data/`, `artifacts/`, or the
local model cache.

Accepted asset states:

- **Assets absent/incomplete:** `validate-assets`, `validate-tokenizer`,
  `run-bonsai`, `run-bonsai-golden`, `run-bonsai-bench`,
  `run-bonsai-q4-golden`, and `run-bonsai-q4-bench` fail nonzero with setup
  guidance naming the missing manifest, asset entry, referenced file, or
  prerequisite golden artifact.
- **Assets present:** the same opt-in gates must emit pass artifacts before
  anyone may claim real Bonsai/Q4 parity.  Q4 acceptance is tied to the
  `qwen3-1.7b-q4-gs64` entry; `qwen3-1.7b-q4` is diagnostic only.

Heavy hitters: `transformer.zig`, `network.zig`, and Metal shader files under
`engine/src/metal/`.
Comptime spine: `engine/src/tensor/layout.zig` and related fixture/model layout
code resolve buffer shapes and offsets as early as practical.
Metal spine: `engine/src/metal/bridge.m`, `shader_catalog.zig`, and shader files
under `engine/src/metal/shaders/` / `engine/src/metal/kernels.metal`.
Agent spine: `labrat/src/agent_core.zig` and `agent_cli_core.zig` provide the
shared offline/live agent loop; domain files configure lanes.
Toolbox spine: `labrat/src/toolbox.zig` provides sandboxed tools; researcher and
agent binaries are thin domain entry points.
...
- Use comptime-known sizes from the layout code to determine buffer capacities at compile time

For `engine/`, this means: parameter buffers, gradient buffers, and activation buffers should have fixed upper bounds allocated at init time. This eliminates allocation jitter during forward/backward passes and prevents GPU stalls caused by buffer reallocation.

### 3. **Assertion Density**

**Minimum 2 assertions per function**. For `engine/`:
...
- **Don't do things directly in reaction to external events.** Your program should run at its own pace — not react to external signals or callbacks synchronously. This keeps control flow under your control, improves performance through batching instead of context switching on every event, and makes it easier to maintain bounds on work done per time period. For `engine/`, this means the training loop drives the tempo: it pulls batches and dispatches kernels on its schedule, not in reaction to Metal completion handlers or data-loading callbacks
...
`engine/` currently avoids external Zig package dependencies. Its platform bindings are explicit:

- **Objective-C bridge (`engine/src/metal/bridge.m`)** — Metal/Foundation interop.
- **Apple Metal framework** — the GPU compute backend.
- **Apple Foundation framework** — Objective-C runtime support around Metal objects.
- **Apple Accelerate framework** — platform math/runtime support where linked by the build.
- **libobjc/libc** — system runtime linkage required by the bridge.
...
`engine/` layout code resolves all buffer sizes, weight/bias offsets, and activation sizes at compile time. Lean into this:
