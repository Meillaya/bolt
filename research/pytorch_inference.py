#!/usr/bin/env python3
"""PyTorch inference benchmark mirroring bolt's inference_bench.zig.
...
Usage:
  python research/pytorch_inference.py
  python research/pytorch_inference.py --save
  python research/pytorch_inference.py --compare "artifacts/inference_*.json"
...
def _build_model_class():
    ...
    class MnistNet(nn.Module):
        ...
        Matches bolt's NetworkLayout exactly.  The last
...
# ── Printing (to stderr, matching bolt's debug output) ──────
...
def compare_with_bolt(
    ...
    """Print side-by-side comparison with an bolt
    ...
    print(file=sys.stderr)
    print(
        f"{'Comparison with bolt':^58}",
        file=sys.stderr,
    )
    print(f"  bolt file: {bolt_path}", file=sys.stderr)
    print(sep, file=sys.stderr)
    print(
        f"{'Metric':<30} {'bolt':>12} {'PyTorch':>12}",
        file=sys.stderr,
    )
    print(sep, file=sys.stderr)

    # GPU batched throughput.
    nz_gpu = bolt.get("gpu_batched")
    ...
    nz_gs = bolt.get("gpu_single_sample")
    ...
    nz_cs = bolt.get("cpu_single_sample")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "PyTorch inference benchmark "
            "(mirrors bolt inference_bench.zig)"
        ),
    )
    parser.add_argument(
        "--save",
        action="store_true",
        help="Save JSON results to artifacts/ directory",
    )
    parser.add_argument(
        "--compare",
        default=None,
        help=(
            "Glob pattern for bolt benchmark JSON files "
            "to compare against"
        ),
    )
    ...
    if args.compare is not None:
        ...
        if not paths:
            ...
            compare_with_bolt(results, paths[-1])
    else:
        # Auto-detect bolt inference benchmarks.
        bolt_files = sorted(
            glob.glob("artifacts/inference_2*.json"),
        )
        if bolt_files:
            compare_with_bolt(results, bolt_files[-1])
