#!/usr/bin/env python3
"""PyTorch reference implementation for bolt MNIST benchmark.
...
Weight init matches bolt's heInit():
  w ~ U(-limit, limit),  limit = sqrt(6 / fan_in)
  b = 0

Usage:
  python research/pytorch_reference.py

  # With a specific data directory:
  python research/pytorch_reference.py --data-dir ./data/mnist_torch

  # Compare against a specific bolt benchmark:
  python research/pytorch_reference.py --compare artifacts/mnist_*.json
...
class MnistNet(nn.Module):
    ...
    Matches bolt's NetworkLayout exactly. The last layer
...
def he_init(model: MnistNet) -> None:
    """He uniform init matching bolt's heInit().
    ...
    leaky_relu slope).  We set it manually to match bolt.
    """
...
def load_datasets(
    ...
    # the partition (same strategy as bolt).
...
def evaluate(
    ...
    Matches bolt's evaluate(): iterates in batches, sums
    ...
    for images, labels in loader:
        ...
        # manually to match bolt's accumulation).
...
def train_epoch(
    ...
    bolt reports train_loss as the loss of the final batch
...
def compare_with_bolt(
    pytorch_results: dict,
    bolt_path: str,
) -> None:
    """Print side-by-side comparison with an bolt benchmark."""
    with open(bolt_path) as f:
        bolt = json.load(f)

    sep = "-" * 54
    print(f"\n{'Comparison with bolt':^54}")
    print(sep)
    print(f"{'Metric':<30} {'bolt':>10} {'PyTorch':>10}")
    print(sep)

    nz_val_acc = bolt["final_validation_accuracy_pct"]
    ...
    nz_val_loss = bolt["final_validation_loss"]
    ...
    nz_test_acc = bolt["final_test_accuracy_pct"]
    ...
    nz_time = bolt["total_training_ms"]
    ...
    nz_tput = bolt["throughput_images_per_sec"]
    ...
    print(sep)
    print(
        f"{'Epoch':<8} {'bolt acc':>10} {'PT acc':>10}"
        f" {'bolt ms':>10} {'PT ms':>10}"
    )
    print(sep)

    nz_epochs = bolt["epochs"]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="PyTorch reference for bolt MNIST benchmark",
    )
    parser.add_argument(
        "--data-dir",
    ...
    parser.add_argument(
        "--compare",
        default=None,
        help="Path to bolt benchmark JSON for comparison",
    )
    parser.add_argument(
        "--save",
        action="store_true",
        help="Save results to artifacts/ as JSON",
    )
    parser.add_argument(
        "--device",
        default=None,
    ...
    # ── Hyperparameters (match bolt exactly) ──
    ...
    test_count = 10_000
    val_interval = 5  # Validate every N epochs (matches bolt).
    ...
    # ── Optimizer ──
    # Plain SGD: no momentum, no weight decay — matches bolt.
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        # last epoch — matches bolt's val_interval logic.
        is_val_epoch = (
    ...
    if args.compare is not None:
        ...
        if not paths:
            ...
            # Use the most recent bolt benchmark.
            bolt_path = sorted(paths)[-1]
            print(f"Comparing with: {bolt_path}")
            compare_with_bolt(results, bolt_path)
    else:
        # Auto-detect bolt benchmarks for comparison.
        import glob

        bolt_files = sorted(
            glob.glob("artifacts/mnist_*.json"),
        )
        if bolt_files:
            bolt_path = bolt_files[-1]
            print(f"Auto-comparing with: {bolt_path}")
            compare_with_bolt(results, bolt_path)
