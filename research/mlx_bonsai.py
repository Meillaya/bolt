#!/usr/bin/env python3
"""MLX inference benchmark for Bonsai 1.7B — bolt comparison.

Mirrors bolt's examples/bonsai_bench.zig — three phases:
...
Usage:
  python research/mlx_bonsai.py
  python research/mlx_bonsai.py --model /path/to/bonsai-1.7b
  python research/mlx_bonsai.py --save
  python research/mlx_bonsai.py --compare "artifacts/bonsai_bench_*.json"
"""
...
def save_json(json_dict: dict) -> Path:
    """Write results to artifacts/bonsai_mlx_<timestamp>.json."""
...
def compare_benchmarks(
    ...
    """Print a side-by-side comparison table against bolt results."""
    ...
    for path in sorted(file_paths):
        ...
        framework = data.get("framework", "bolt")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "mirrors bolt's bonsai_bench.zig"
        ),
    )
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(e.g. 'artifacts/bonsai_bench_*.json')"
        ),
    )
