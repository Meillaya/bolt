#!/usr/bin/env python3
"""MLX inference benchmark for bolt comparison.

Mirrors bolt's examples/inference_bench.zig — three phases:
...
Usage:
  python research/mlx_inference.py
  python research/mlx_inference.py --save
  python research/mlx_inference.py --compare "artifacts/inference_*.json"
...
def compute_stats(latencies_us: list[float]) -> dict:
    ...
    # Nearest-rank percentile (same as bolt).
    p50_idx = n * 50 // 100
    p99_idx = n * 99 // 100
...
def compare_benchmarks(
    ...
    """Print comparison table against bolt benchmark files."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "MLX inference benchmark — mirrors bolt's "
            "inference_bench.zig"
        ),
    )
    parser.add_argument(
    ...
    # ── Input data (random, same as bolt) ──
    # bolt fills input buffers with random floats in [0, 1).
