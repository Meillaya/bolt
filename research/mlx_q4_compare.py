#!/usr/bin/env python3
"""Compare MLX Q4 logits with bolt.

This script:
1. Encodes the prompt using BOTH bolt's template and MLX's template
2. Runs a forward pass with MLX using bolt's 13-token prompt
...
Usage:
  python research/mlx_q4_compare.py
  python research/mlx_q4_compare.py --model /path/to/qwen3-1.7b-q4
  python research/mlx_q4_compare.py --use-mlx-template
"""
...
DEFAULT_MODEL_DIRS = [
    Path(__file__).resolve().parents[1] / "data" / "qwen3-1.7b-q4-gs64",
    Path(__file__).resolve().parents[1] / "data" / "qwen3-1.7b-q4",
    Path.home() / "models" / "qwen3-1.7b-q4-gs64",
    Path.home() / "models" / "qwen3-1.7b-q4",
]


def resolve_model_dir(override: str | None) -> str:
    if override is not None:
        if not Path(override).expanduser().is_dir():
...
def build_bolt_prompt(tokenizer) -> list[int]:
    """Replicate bolt's applyChatTemplate exactly:
    ...
    # Encode sub-parts the same way bolt does:
    # bolt calls tokenizer.encode() on each piece separately.
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare MLX Q4 logits with bolt",
    )
    ...
    # Build prompts
    bolt_ids = build_bolt_prompt(tokenizer)
    mlx_ids = build_mlx_prompt(tokenizer)
    mlx_think_ids = build_mlx_thinking_prompt(tokenizer)

    print(f"\n--- Prompt comparison ---")
    print(f"bolt template ({len(bolt_ids)} tokens): {bolt_ids}")
    print(f"  text: {repr(tokenizer.decode(bolt_ids, skip_special_tokens=False))}")
    ...
    dump_embedding_values(model, bolt_ids)

    # Generate with bolt's template (13 tokens)
    bolt_output = generate_with_logit_dump(
        model, tokenizer, bolt_ids, MAX_TOKENS,
        "bolt-template",
    )
    ...
    print(f"  SUMMARY")
    print(f"{'=' * 60}")
    print(f"  bolt template ({len(bolt_ids)} tok prompt) -> "
          f"{len(bolt_output)} tok output: {bolt_output}")
