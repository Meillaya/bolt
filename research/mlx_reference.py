#!/usr/bin/env python3
"""MLX reference implementation for bolt MNIST benchmark.

MLX is Apple's array framework for Apple Silicon — it runs on
the same Metal GPU with unified memory that bolt targets.
...
data/mnist_torch/MNIST/raw/ — the same files bolt reads. No torchvision
download required.

Usage:
  python research/mlx_reference.py

  # Compare against bolt and/or PyTorch benchmarks:
  python research/mlx_reference.py --compare "artifacts/mnist_*.json"

  # Save results:
  python research/mlx_reference.py --save
...
# ── MNIST IDX loader ─────────────────────────────────────────
#
# Reads the same binary files bolt uses (data/mnist_torch/MNIST/raw/).
...
def evaluate(
    ...
    Matches bolt's evaluate() semantics.
    """
...
def find_benchmarks() -> dict[str, str]:
    """Find the most recent bolt and PyTorch benchmarks."""
    found = {}

    bolt_files = sorted(glob.glob("artifacts/mnist_*.json"))
    if bolt_files:
        found["bolt"] = bolt_files[-1]

    pytorch_files = sorted(glob.glob("artifacts/pytorch_mnist_*.json"))
    if pytorch_files:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="MLX reference for bolt MNIST benchmark",
    )
    parser.add_argument(
        "--data-dir",
        default="data/mnist",
        help=(
            ...
            "(default: data/mnist_torch/MNIST/raw — same as bolt)"
        ),
    )
    parser.add_argument(
        "--compare",
    ...
    # freeze the partition (same strategy as bolt).
    mx.random.seed(seed)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        # Reshuffle training indices each epoch (bolt does
        ...
        # last epoch — matches bolt's val_interval=5.
        is_val_epoch = (
