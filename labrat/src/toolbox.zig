...
//!   run-cmd             Run an allowlisted bounded command
...
const MAX_TOOL_RETURN_BYTES: usize = 50_000;
const MAX_RUN_ARGS: usize = 16;
...
/// Caller owns cleanup; this function never deletes caller-supplied paths.
fn readJsonInput(
    arena: Allocator,
    args: []const []const u8,
) ?std.json.ObjectMap {
...
fn dispatch(
    ...
    if (tools.eql(cmd, "copy-file")) {
        return toolCopyFile(config, arena, obj);
    }
    ...
    if (tools.eql(cmd, "run-cmd")) {
        return toolRunCmd(config, arena, obj);
    }
...
fn runAndReport(
    ...
    const result = runProcessWithTimeout(
        arena,
        argv,
        config.project_root,
    ...
    const success = result.exit_code != null and
...
fn runExtraBench(
    ...
    if (result.output.len > 0) {
        try tools.writeStdout(result.output);
    } else {
        try tools.writeStdout(
            "{\"status\": \"error\", " ++
                "\"error\": \"no benchmark " ++
...
fn scanForFunction(
    ...
    var brace_state = BraceScanState{};
    ...
    while (iter.next()) |line| {
        ...
        // Count braces on this line with lexical state carried
        // across lines, so multiline block comments cannot corrupt
        // function bounds.
        depth = countBracesOnLineState(line, depth, &brace_state);
...
pub fn countBracesOnLine(
    ...
    return countBracesOnLineState(line, depth, &state);
}

fn countBracesOnLineState(
    line: []const u8,
    depth: i32,
    ...
    while (i < line.len) : (i += 1) {
        const c = line[i];
        const next = if (i + 1 < line.len) line[i + 1] else 0;
        ...
        if (!state.in_string and !state.in_char and c == '/' and next == '/') break;
        if (!state.in_string and !state.in_char and c == '/' and next == '*') {
...
fn toolWriteFile(
    ...
    try writeSnapshot(config, arena, path, fs_path);
...
fn toolCopyFile(
    ...
    const source = getJsonString(obj, "source") orelse {
        try writeJsonErrorMsg(arena, "missing 'source' field");
        std.process.exit(1);
    };
    const dest = getJsonString(obj, "dest") orelse {
        try writeJsonErrorMsg(arena, "missing 'dest' field");
        std.process.exit(1);
    };
    if (!isAllowedReadPath(config, source)) {
        const msg = try std.fmt.allocPrint(
            arena,
            "copy source not allowed for '{s}'",
            .{source},
        );
        try writeJsonErrorMsg(arena, msg);
        std.process.exit(1);
    }
    if (!isAllowedWritePath(config, dest)) {
        const msg = try std.fmt.allocPrint(
            arena,
            "copy dest not allowed for '{s}'",
            .{dest},
        );
        try writeJsonErrorMsg(arena, msg);
        std.process.exit(1);
    }
    const source_fs = try tools.resolveToFs(arena, config.fs_root, source);
    const dest_fs = try tools.resolveToFs(arena, config.fs_root, dest);
    try writeSnapshot(config, arena, dest, dest_fs);
    try tools.copyFile(source_fs, dest_fs);
    const ok_msg = try std.fmt.allocPrint(
        arena,
        "{{\"status\": \"ok\", " ++
            "\"source\": \"{s}\", " ++
...
fn toolListDir(
    ...
    const ok = result.exit_code != null and result.exit_code.? == 0 and !result.timed_out and !result.spawn_error;
    if (result.output.len > 0) {
        const escaped = try tools.jsonEscape(
            arena,
            result.output,
...
// ============================================================
// Tool: run-cmd — run an allowlisted command
// ============================================================
...
fn toolRunCmd(
    ...
    if (!isAllowedRunCommand(command)) {
        try writeJsonErrorMsg(
            arena,
    ...
    const result = runAllowedCommand(
        arena,
        command,
        config.project_root,
    );
    ...
    const exit_str = if (result.exit_code) |code|
        ...
        "null";
    const status = if (result.exit_code != null and result.exit_code.? == 0 and !result.timed_out and !result.spawn_error)
        "ok"
    else
        "error";
    const json = try std.fmt.allocPrint(
        ...
        "{{\"status\": \"{s}\", " ++
            ...
            "\"timed_out\": {}, " ++
            "\"output\": \"{s}\"}}\n",
        .{ status, exit_str, result.timed_out, escaped },
    );
    try tools.writeStdout(json);
    if (!tools.eql(status, "ok")) std.process.exit(1);
}
...
const DEFAULT_PROCESS_TIMEOUT_SECONDS: i64 = 120;
const BENCH_PROCESS_TIMEOUT_SECONDS: i64 = 900;

fn isAllowedRunCommand(command: []const u8) bool {
    ...
    const allowed = [_][]const u8{
        ...
        "zig build run-bonsai-golden -Doptimize=ReleaseFast",
        "zig build run-bonsai-bench -Doptimize=ReleaseFast",
        "zig build run-bonsai-q4-golden -Doptimize=ReleaseFast",
        "zig build run-bonsai-q4-bench -Doptimize=ReleaseFast",
    };
    for (allowed) |item| {
        if (tools.eql(command, item)) return true;
    }
    return false;
}

fn runAllowedCommand(
    arena: Allocator,
    command: []const u8,
    cwd: []const u8,
) ShellResult {
    var argv_buf: [MAX_RUN_ARGS][]const u8 = undefined;
    const argv = splitAllowedCommand(command, &argv_buf);
...
fn splitAllowedCommand(
    command: []const u8,
    argv_buf: *[MAX_RUN_ARGS][]const u8,
) []const []const u8 {
    var count: usize = 0;
    var iter = std.mem.splitScalar(u8, command, ' ');
    while (iter.next()) |part| {
        if (part.len == 0) continue;
        std.debug.assert(count < MAX_RUN_ARGS);
        argv_buf[count] = part;
        count += 1;
    }
    std.debug.assert(count > 0);
    return argv_buf[0..count];
}

/// Execute an internal maintenance shell command. This is not
/// reachable from the user-provided run-cmd tool.
fn runShellCapture(
    arena: Allocator,
    command: []const u8,
) ShellResult {
...
fn runProcessWithTimeout(
    ...
    var env_argv_buf: [MAX_RUN_ARGS + 1][]const u8 = undefined;
    var run_argv = argv;
    if (std.mem.indexOfScalar(u8, argv[0], '/') == null) {
        env_argv_buf[0] = "/usr/bin/env";
        for (argv, 0..) |arg, i| env_argv_buf[i + 1] = arg;
        run_argv = env_argv_buf[0 .. argv.len + 1];
    }

    const result = std.process.run(arena, io_state.io(), .{
        .argv = run_argv,
        .cwd = .{ .path = cwd },
        ...
        return .{
            .output = msg,
            .exit_code = null,
            .timed_out = err == error.Timeout,
            .spawn_error = err != error.Timeout,
        };
    };
    ...
    return .{
        .output = combineOutput(arena, result.stdout, result.stderr),
        .exit_code = exit_code,
    };
}
...
fn emitShellFailure(
    ...
    const json = try std.fmt.allocPrint(
        ...
        "{{\"status\": \"error\", " ++
            ...
            "\"timed_out\": {}, " ++
            "\"output\": \"{s}\"}}\n",
        .{ tag, exit_str, result.timed_out, escaped },
    );
    try tools.writeStdout(json);
}
...
fn truncateToolOutput(output: []const u8) []const u8 {
    if (output.len <= MAX_TOOL_RETURN_BYTES) return output;
    return output[0..MAX_TOOL_RETURN_BYTES];
}
...
fn toolExperimentStart(
    ...
    if (result.exit_code == null or result.exit_code.? != 0 or result.timed_out or result.spawn_error) {
        ...
        const json = try std.fmt.allocPrint(
            arena,
            "{{\"status\": \"error\", " ++
                "\"error\": \"experiment_start_failed\", " ++
                "\"timed_out\": {}, " ++
                "\"output\": \"{s}\"}}\n",
            .{ result.timed_out, escaped },
        );
        try tools.writeStdout(json);
        std.process.exit(1);
    }
...
fn toolExperimentFinish(
    ...
    // 3. Allocate the future history id. Durable history is appended
    // only after the git transition succeeds, so audit records cannot
    // claim success for a failed rollback/merge.
    const exp_num =
    ...
    if (is_keep) {
        ...
        const msg_file = try std.Io.Dir.cwd().createFile(
            msg_path,
            .{},
        );
        ...
        const result = runShellCapture(arena, cmd);
        if (result.exit_code == null or result.exit_code.? != 0 or result.timed_out or result.spawn_error) {
            try emitShellFailure(arena, "experiment_finish_keep_failed", result);
            std.process.exit(1);
        }
    } else {
        const result = runShellCapture(
        ...
        if (result.exit_code == null or result.exit_code.? != 0 or result.timed_out or result.spawn_error) {
            try emitShellFailure(arena, "experiment_finish_abandon_failed", result);
            std.process.exit(1);
        }
    }
...
fn cleanLastBench(
    ...
    std.Io.Dir.cwd().deleteFile(fs_path) catch {};
}
...
fn writeSnapshot(
    ...
    const snap_dir_rel = try std.fmt.allocPrint(
        arena,
        "{s}/_snapshots",
        .{config.history_dir},
    );
    const snap_dir = try tools.resolveToFs(
        arena,
        config.fs_root,
        snap_dir_rel,
    );
    try std.Io.Dir.cwd().makePath(snap_dir);

    const had_original = tools.fileExists(fs_path);
    const original = if (had_original)
        tools.readFile(arena, fs_path) catch ""
    else
        "";
    const escaped_path = try tools.jsonEscape(arena, logical_path);
    const escaped_content = try tools.jsonEscape(arena, original);
    var ts_buf: [32]u8 = undefined;
    const ts = formatTimestampUtc(&ts_buf);
    const safe = sanitizeBranchName(arena, logical_path);
    ...
    const json = try std.fmt.allocPrint(
        arena,
        "{{\"path\":\"{s}\",\"had_original\":{}," ++
            "\"original_len\":{d},\"content\":\"{s}\"}}\n",
        .{ escaped_path, had_original, original.len, escaped_content },
    );
    try tools.writeFile(snap_path, json);
}
...
fn isAllowedReadPath(
    ...
    if (pathHasSymlinkComponent(config, path)) return false;
    if (isReadonlyLedgerPath(path)) return true;
...
fn isAllowedWritePath(
    ...
    std.debug.assert(path.len > 0);
    if (isDeniedPath(path)) return false;
    if (isReadonlyLedgerPath(path)) return false;
    if (pathHasSymlinkComponent(config, path)) return false;
    for (config.write_scope) |prefix| {
        if (tools.startsWith(path, prefix)) {
            return true;
...
fn pathHasSymlinkComponent(
    ...
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    const fs_path = tools.resolveToFs(arena, config.fs_root, path) catch return true;
    var io_state = std.Io.Threaded.init(std.heap.page_allocator, .{});
    defer io_state.deinit();

    var i: usize = 0;
    while (i < fs_path.len) : (i += 1) {
        if (fs_path[i] != '/') continue;
        if (i == 0) continue;
        if (componentIsSymlink(io_state.io(), fs_path[0..i])) return true;
    }
    return componentIsSymlink(io_state.io(), fs_path);
}

fn componentIsSymlink(io: std.Io, sub_path: []const u8) bool {
...
fn isDeniedPath(path: []const u8) bool {
...
fn isReadonlyLedgerPath(path: []const u8) bool {
    return tools.eql(path, ".omx/ultragoal/ledger.jsonl");
}
...
// ------------------------------------------------------------
// countBracesOnLine — comment and literal brace handling
//
// Braces in strings, chars, line comments, and block comments
// are ignored so function-boundary scanning stays stable when
// comments contain example braces.
// ------------------------------------------------------------

test "countBracesOnLine: brace in line comment ignored" {
    ...
    // Braces in line comments are ignored.
    try std.testing.expectEqual(@as(i32, 1), depth);
}

test "countBracesOnLine: open brace in line comment ignored" {
    const depth = countBracesOnLine(
        "    // see note about {",
        1,
    );
...
test "countBracesOnLine: Metal block comment braces ignored" {
    // Metal shaders use /* */ comments.
    const depth = countBracesOnLine(
        "    /* threadgroup buffer { shared } */",
        1,
    );
    // Braces in block comments are ignored.
    try std.testing.expectEqual(@as(i32, 1), depth);
}

test "countBracesOnLine: unbalanced brace in block comment ignored" {
    const depth = countBracesOnLine(
        "    /* see { for details */",
        1,
    );
...
test "countBracesOnLine: char literal brace ignored" {
    // Zig char literal: '{' should not count.
    const depth = countBracesOnLine(
        "    const c = '{';",
        0,
    );
    // Braces in single-quoted literals are ignored.
    try std.testing.expectEqual(@as(i32, 0), depth);
}
...
// ------------------------------------------------------------
// scanForFunction — comment braces do not break bounds
//
// Comments frequently contain example braces. The scanner keeps
// comment state across lines so line and block comments cannot
// truncate or extend function bounds.
// ------------------------------------------------------------

test "scanForFunction: comment with closing brace does not truncate function" {
    // A function whose body contains a comment with }.
    // The parser ignores the } in the comment.
    const content =
    ...
    try std.testing.expect(bounds != null);
    // The comment brace is ignored, so the function ends at its real closing brace.
    try std.testing.expectEqual(
        @as(u32, 5),
        bounds.?.end_line,
    );
...
test "scanForFunction: comment with open brace does not extend function" {
    // The { in the comment is ignored, so the real closing brace
    // still ends the function before the next declaration.
    const content =
        \\fn alpha() void {
    ...
    try std.testing.expect(bounds != null);
    // The line comment brace is ignored, so the function ends at
    // its real closing brace instead of extending into beta().
    try std.testing.expectEqual(
        @as(u32, 4),
        bounds.?.end_line,
    );
...
test "scanForFunction: multiline block comment braces ignored" {
    const content =
        \\fn render(self: *Self) void {
        \\    /* opening block comment
        \\       misleading closing brace }
        \\    */
        \\    self.present();
        \\}
    ;
    const bounds = scanForFunction(content, "fn render(");
    try std.testing.expect(bounds != null);
    try std.testing.expectEqual(@as(u32, 6), bounds.?.end_line);
}
...
// ------------------------------------------------------------
// Labrat Phase 2 sandbox hardening tests
// ------------------------------------------------------------

test "labrat sandbox denies protected roots" {
    const config = ToolboxConfig{
        .name = "test",
        .project_root = ".",
        .write_scope = &.{"engine/src/"},
        .read_scope = &.{ "engine/", "docs/" },
        .read_files = &.{"README.md"},
        .check_command = &.{ "zig", "build" },
        .test_command = &.{ "zig", "build", "test" },
        .bench_command = &.{ "zig", "build", "run" },
        .history_dir = "artifacts/labrat-history/test",
    };
...
test "labrat run-cmd accepts only stable argv commands" {
    ...
    try std.testing.expect(!isAllowedRunCommand("curl https://example.invalid"));
}

test "labrat command splitter preserves argv boundaries" {
    var argv_buf: [MAX_RUN_ARGS][]const u8 = undefined;
    const argv = splitAllowedCommand(
        "zig build run-bonsai-q4-golden -Doptimize=ReleaseFast",
        &argv_buf,
    );
    try std.testing.expectEqual(@as(usize, 4), argv.len);
    try std.testing.expect(tools.eql(argv[0], "zig"));
    try std.testing.expect(tools.eql(argv[1], "build"));
    try std.testing.expect(tools.eql(argv[2], "run-bonsai-q4-golden"));
    try std.testing.expect(tools.eql(argv[3], "-Doptimize=ReleaseFast"));
}

test "labrat -f JSON input does not delete caller file" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    const path = "zig-cache/tmp/labrat-json-input-test.json";
    var io_state = std.Io.Threaded.init(std.heap.page_allocator, .{});
    defer io_state.deinit();
    const io = io_state.io();
    try std.Io.Dir.cwd().createDirPath(io, "zig-cache/tmp");
    const file = try std.Io.Dir.cwd().createFile(io, path, .{});
    defer file.close(io);
    try file.writeStreamingAll(io, "{\"tool\":\"cwd\"}");
    const obj = readJsonInput(arena, &.{ "-f", path });
    try std.testing.expect(obj != null);
    try std.testing.expect(tools.fileExists(path));
    std.Io.Dir.cwd().deleteFile(io, path) catch {};
}

test "labrat path guard rejects symlink components" {
    const config = ToolboxConfig{
        .name = "test",
        .project_root = ".",
        .fs_root = ".",
        .write_scope = &.{"zig-cache/tmp/labrat-sandbox/"},
        .read_scope = &.{"zig-cache/tmp/labrat-sandbox/"},
        .read_files = &.{},
        .check_command = &.{ "zig", "build" },
        .test_command = &.{ "zig", "build", "test" },
        .bench_command = &.{ "zig", "build", "run" },
        .history_dir = "zig-cache/tmp/labrat-history/test",
    };
    var io_state = std.Io.Threaded.init(std.heap.page_allocator, .{});
    defer io_state.deinit();
    const io = io_state.io();
    try std.Io.Dir.cwd().createDirPath(io, "zig-cache/tmp/labrat-sandbox");
    std.Io.Dir.cwd().deleteFile(io, "zig-cache/tmp/labrat-sandbox/link") catch {};
    try std.Io.Dir.cwd().symLink(io, "/tmp", "zig-cache/tmp/labrat-sandbox/link", .{});
    defer std.Io.Dir.cwd().deleteFile(io, "zig-cache/tmp/labrat-sandbox/link") catch {};
    try std.testing.expect(!isAllowedReadPath(&config, "zig-cache/tmp/labrat-sandbox/link/file.txt"));
